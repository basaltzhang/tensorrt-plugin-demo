...
pluginStatus_t nonMaxSuppressionInference(cudaStream_t stream, const int N, const int perBatchBoxesSize, const int perBatchScoresSize,
    ...
    void* nmsedIndices, void* workspace, bool isNormalized, bool confSigmoid)
{
    ...
    // cudaMemcpy(a, postNMSIndices, 200 * sizeof(int), cudaMemcpyDeviceToHost);
    // for (int b = 0; b < 100; b ++) {
    //     std::cout << ", " << a[b];
    // }
    // std::cout  << std::endl;
    ...
    status = gatherNMSOutputs(stream, shareLocation, N, numPredsPerClass, numClasses, topK, keepTopK, DataType::kFLOAT,
        DataType::kFLOAT, indices, scores, bboxData, nmsedIndices);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
